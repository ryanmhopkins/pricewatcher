...
create table if not exists snapshots(id uuid primary key default gen_random_uuid(),monitor_id uuid not null references monitors(id) on delete cascade,page_title text,content_hash text not null,pricing_text text not null,screenshot_path text,created_at timestamptz not null default now());
